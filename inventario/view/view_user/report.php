...
<?php 
	...
	if(isset($_POST['generar_pdf'])){
		...
		if(isset($_POST['responsable'])){ $responsable = $_POST['responsable']; }else{ echo $responsable=''; };
		if(isset($_POST['area'])){ $area = $_POST['area']; }else{ echo $area=''; };

		if($reporte->generar_pdf($inicio,$fin,$area,$responsable)){

			printf('<script type="text/javascript">alert("Se ha generado el  reporte correctamente.")</script>');
	...
	<script language='javascript'> 
	   ...
	   function accion(){ 
	    with (document.frm){ 
	     ...
	     switch (responsable.selectedIndex){ 
	      case 0: 
	       area.disabled = false; 
	       break;
	      case 1: 
	    ...
	    <div class="row d-flex d-md-block flex-nowrap wrapper">
	        ...
	        <main class="col-md-9 float-left col px-5 pl-md-2 pt-2 main">
	            <div class="container" style="padding: 20px;">
	              <h1 class="display-5">Inventario. Reporte</h1>
		          <form method="POST" name='frm'>
		          	<div class="form-row">
		          		<div class="form-group col-lg-6">
						    <label for="inicio">Inicio:</label>
						    <input type="date" class="form-control" name="inicio"  autocomplete="off" maxlength="255" required="required">
						  </div>
						  <div class="form-group col-lg-6">
						    <label for="fin">Fin:</label>
						    <input type="date" class="form-control" name="fin"  autocomplete="off" maxlength="255" required="required">
					  	</div>
		          	</div>
		          	<div class="form-row">
		          		 <div class="form-group col-lg-6">
						  	<label for="area">Area:</label>
						  	<select class="form-control" name="area" onchange='accion()'>
						  	<option value="" selected=""></option>
						  	<?php  
						  		$nombre = $_SESSION['nombre'];
						  		$query = "SELECT a.id,a.nombre FROM area a INNER JOIN responsable r ON a.id=r.id_a WHERE r.nombre = '$nombre'";
						  		$reporte->view_area($query);
						  ...
						  <div class="form-group col-lg-6">
						  	...
						  	<select class="form-control" name="responsable" onchange='accion()'>
						  	<option value="" selected=""></option>
						  	<?php  

						  		$nombre = $_SESSION['nombre'];
							  	$query = "SELECT * FROM responsable WHERE nombre = '$nombre'";
						  		$reporte->view_responsable($query);
					...
					<div class="form-group">
					    <button type="submit" class="btn btn-primary" name="generar_pdf">Generar Reporte</button>
					</div>
				</form>
	            </div>
	        </main>
